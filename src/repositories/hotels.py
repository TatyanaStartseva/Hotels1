...
class HotelsRepository(BaseRepository):
    ...
    async def get_filtered_by_time(self, date_from:date, date_to: date, location,title,limit,offset):
        ...
        if location:
            query = query.filter(self.model.location.ilike(
                f"%{location}%"))  # ilike чтобы при вводе данных они были регистро независимые
        query = (
            query
            .limit(limit)
            .offset(offset)
